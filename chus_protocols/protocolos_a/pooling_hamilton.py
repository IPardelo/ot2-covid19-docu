    ...
    for sources, dest in zip(custom_sources, destinations):
        for source in sources:
            ...
            common.move_vol_multichannel(ctx, p300, reagent=sample, source=source, dest=dest, vol=volume_to_be_transfered / pooling_factor,
                                         air_gap_vol=air_gap_vol_sample, x_offset=x_offset, pickup_height=pickup_height,
                                         rinse=sample.get('rinse'), disp_height=dispense_height, blow_out=True, touch_tip=True)
            # Drop pipette tip
            p300.drop_tip()
